...
KGClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://kg-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e) {
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://kg-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://kg-clothing/setCurrentPed', JSON.stringify({ ped: newValue }), function(model) {
                    $("#current-model").html("<p>" + model + "</p>")
                });
                canChange = true;
                KGClothing.ResetValues()
                ...
                if (buttonType == "item") {
                    KGClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function() {
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://kg-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e) {
    ...
    if (selectedCam == null) {
        ...
        $.post('https://kg-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch (event.keyCode) {
        case 68: // D
            $.post('https://kg-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://kg-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
KGClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function() {
    window.addEventListener('message', function(event) {
        ...
        switch (event.data.action) {
            case "open":
                KGClothing.Open(event.data);
                break;
            case "close":
                KGClothing.Close();
                break;
            case "updateMax":
                KGClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                KGClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                KGClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                KGClothing.ResetValues();
                break;
...
KGClothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e) {
    e.preventDefault();
    KGClothing.Close();
    $.post('https://kg-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e) {
    e.preventDefault();
    KGClothing.Close();
    $.post('https://kg-clothing/resetOutfit');
});

KGClothing.SetCurrentValues = function(clothingValues) {
...
KGClothing.Open = function(data) {
    ...
    KGClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    KGClothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e) {
    ...
    $.post('https://kg-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e) {
    ...
    $.post('https://kg-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
KGClothing.Close = function() {
    $.post('https://kg-clothing/close');
...
KGClothing.SetMaxValues = function(maxValues) {
...
KGClothing.ResetValues = function() {
...
$(document).on('click', '#save-outfit-save', function(e) {
    ...
    $.post('https://kg-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e) {
    ...
    $.post('https://kg-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
