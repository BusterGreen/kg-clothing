...
local function SaveSkin()
    ...
    TriggerServerEvent("kg-clothing:saveSkin", model, clothing)
end
...
local function reloadSkin(health)
    ...
    local gender = KGCore.Functions.GetPlayerData().charinfo.gender
    ...
    TriggerServerEvent("kg-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("kg-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
...
local function getOutfits(gradeLevel, data)
    local gender = "male"
    if KGCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    KGCore.Functions.TriggerCallback('kg-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('kg-clothing:client:openMenu', function()
...
RegisterNetEvent('kg-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
        action = "reloadMyOutfits",
        outfits = myOutfits
    })
end)
RegisterNetEvent('kg-clothes:client:CreateFirstCharacter', function()
    KGCore.Functions.GetPlayerData(function(pData)
...
RegisterNetEvent("kg-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('kg-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('kg-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('kg-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if KGCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        KGCore.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
RegisterNetEvent("kg-clothing:client:adjustfacewear", function(type)
    if KGCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
RegisterNetEvent('KGCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("kg-clothes:loadPlayerSkin")
    PlayerData = KGCore.Functions.GetPlayerData()
    loadStores()
--    KGCore.Shared.Jobs = exports['kg-jobs']:AddJobs()
end)
RegisterNetEvent('KGCore:Client:OnJobUpdate', function(JobInfo)
    PlayerData.job = JobInfo
end)
RegisterNetEvent('KGCore:Client:OnGangUpdate', function(GangInfo)
    PlayerData.gang = GangInfo
end)
RegisterNetEvent('kg-clothing:client:openOutfitMenu', function()
    KGCore.Functions.TriggerCallback('kg-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('kg-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNUICallback('TrackerError', function(_, cb)
    KGCore.Functions.Notify(Lang:t("notify.error_bracelet"), "error")
    cb('ok')
end)
RegisterNUICallback('saveOutfit', function(data, cb)
    local ped = PlayerPedId()
    local model = GetEntityModel(ped)
    TriggerServerEvent('kg-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('kg-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('kg-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    KGCore.Functions.Notify(Lang:t('notify.info_deleteOutfit', {outfit = data.outfitName}))
    cb('ok')
end)
    ...
    if Config.UseTarget then
        CreateThread(function()
            for k, v in pairs(Config.Stores) do
                ...
                exports['kg-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                    name = v.shopType .. k,
                    debugPoly = false,
                    minZ = v.coords.z-1,
                    maxZ = v.coords.z+1,
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                elseif not KGCore.Shared.KGJobsStatus then
                    action = function()
                        customCamLocation = v.cameraLocation
                        local gradeLevel = PlayerData.job.grade.level
                        getOutfits(gradeLevel, Config.Outfits[v.requiredJob])
                    end
                else break end --this break is important if KGJobsStatus is true then the reste of the code in this loop does not need to run.

                exports['kg-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
            ...
            clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    ...
                    if zoneName == 'surgeon' then
                        exports['kg-core']:DrawText('[E] - '..Lang:t("store.surgeon"), 'left')
                    elseif zoneName == 'clothing' then
                        exports['kg-core']:DrawText('[E] - '..Lang:t("store.clothing"), 'left')
                    elseif zoneName == 'barber' then
                        exports['kg-core']:DrawText('[E] - '..Lang:t("store.barber"), 'left')
                    elseif zoneName == 'outfit' then
                        exports['kg-core']:DrawText('[E] - '..Lang:t("store.outfitchanger"), 'left')
                    end
                else
                    inZone = false
                    ...
                    if PlayerData.gang and PlayerData.gang.name or (not KGCore.Shared.KGJobsStatus and PlayerData.job.name) then
                        ...
                        clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
                            if isPointInside then
                                local zoneID = tonumber(KGCore.Shared.SplitStr(zone.name, "_")[2])
                                local job = Config.ClothingRooms[zoneID].isGang and PlayerData.gang.name or (not KGCore.Shared.KGJobsStatus and PlayerData.job.name)
                ...
                if inZone then
                    ...
                    elseif not KGCore.Shared.KGJobsStatus then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local gradeLevel = clothingRoom.isGang and PlayerData.gang.grade.level or (not KGCore.Shared.KGJobsStatus and PlayerData.job.grade.level)
